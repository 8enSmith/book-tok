  ...
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        const bottomColor = sampleRegion(ctx, 0, canvas.height * 2/3, canvas.width, canvas.height / 3);
 ...
 */
function sampleRegion(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number): string {
